...
public class TC03_POST_Employee_Record extends TestBase
{
	...
	{
		logger.info("************* Started TC03_POST_Employee_Record ***************");
		...
		RestAssured.given().config(RestAssured.config().encoderConfig(EncoderConfig.encoderConfig()
			...
			.contentType("multipart/form-data; boundary=--MyBoundary").queryParams(requestParams).post("/create");
	...
	void tearDown()
	{
		httpRequest=null;
		response=null;
		logger.info("************* Finishing TC03_POST_Employee_Record ***************");
	}
